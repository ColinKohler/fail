...
class SO2ImplicitPolicy(BasePolicy):
    def __init__(
        ...
        super().__init__(
            obs_dim, action_dim, num_obs_steps, num_action_steps, horizon, z_dim
        )
        self.Lmax = lmax
        ...
        self.in_type = enn.FieldType(self.gspace, [self.gspace.irrep(1)] * 39)

        out_type = enn.FieldType(self.gspace, [self.G.irrep(0)])
        rho = self.G.spectral_regular_representation(
            *self.G.bl_irreps(L=self.Lmax), name=None
        )
        mid_type = enn.FieldType(self.gspace, z_dim * [rho])
        self.energy_mlp = enn.SequentialModule(
            enn.Linear(self.in_type, mid_type),
            enn.FourierPointwise(
                self.gspace, z_dim, self.G.bl_irreps(L=self.Lmax), type="regular", N=16
            ),
    ...
    def get_action(self, obs, device):
        ...
        samples = action_dist.sample((B, self.pred_n_samples, Ta)).to(dtype=nobs.dtype)
