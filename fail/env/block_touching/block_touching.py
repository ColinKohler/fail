...
class RewardSystem(LeafSystem):
  ...
  def getReward(self, context, output):
    ...
    reward = np.allclose(peg_xyz[:2], block_xyz[:2], atol=2e-2)

    output[0] = reward

class BlockTouchingSim(object):
  ...
  def initScene(self, debug=False):
    ...
    block = parser.AddModels(str(ws_path / 'models/block_no_collision.sdf'))[0]
...
class BlockTouching(BaseEnv):
    def __init__(self, meshcat=None, time_limit=GYM_TIME_LIMIT):
        drake_sim = BlockTouchingSim(meshcat=meshcat, time_limit=time_limit)
        ...
        # Define observation space: robot_state (Ex, Ey, Ez, Fx, Fy, Fz, Mx, My, Mz) + world_state (Bx, By, Bz)
        ws_min = [0.4, -0.1, 0.0]
