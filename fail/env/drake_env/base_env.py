...
class BaseEnv(gym.Env):
  ...
  def _setup(self):
    ...
    self.observation_ports = [getOutputPort(system, id) for id in self.observation_port_ids]
  ...
  def step(self, d_action):
    ...
    poses = self.drake_sim.pose_view(self.drake_sim.plant.GetOutputPort('body_poses').Eval(plant_context))
    ...
    terminated = (not timeout and (status.reason() == SimulatorStatus.ReturnReason.kReachedTerminationCondition))
  ...
  def render(self, mode):
    ...
    if mode == 'human':
        self.drake_sim.simulator.get_system().ForcedPublish(self.drake_sim.simulator.get_context())
    elif mode == 'rgb_array':
        rgb = self.rgb_port.Eval(self.drake_sim.simulator.get_context()).data[:,:,:3]
        return rgb
    else:
