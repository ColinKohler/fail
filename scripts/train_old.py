...
def train(
    ...
    lr_scheduler = torch_utils.CosineWarmupScheduler(
        optimizer, 500, len(dataloader) * epochs
    )
        ...
        for epoch_idx in tglobal:
            ...
            with tqdm(dataloader, desc="Batch", leave=False) as tepoch:
                for nbatch in tepoch:
                    nobs, naction = nbatch["obs"].float().to(device), nbatch[
                        "action"
                    ].float().to(device)
                    ...
                    obs[:, :, :3] = (
                        ngoal[:, 0, :].unsqueeze(1).repeat(1, seq_len, 1)
                        - obs[:, :, :3]
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "epochs", type=int, help="Number of epochs to train the policy."
    )
    parser.add_argument("data_path", type=str, help="Filepath to the expert data.")
