...
class FrankaFTPublisher(Node):
    ...
    def __init__(self):
        ...
        self.franka_joint_names = tuple(f"{self.joint_names_prefix}_joint{i}" for i in range(1, 8))
        ...
        self.publisher = self.create_publisher(
            WrenchMessage,
            self.ARM_WRENCH_STATE_TOPIC,
            10
        )
        self.wrench_timer = self.create_timer(
            1 / self.frequency,
            self.publish_wrench_state,
            callback_group=callback_group
        )
    ...
    def joint_state_callback(self, msg: FrankaJointState):
        """ Arm state callback. """
        self.arm_ext_torques.append(np.array(msg.tau_ext[: len(self.franka_joint_names)]))
